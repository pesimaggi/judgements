...
- **Optional accounts and saved judgments** — Clerk manages passwordless email-code authentication; Google and Microsoft are deferred. Search, filters, sorting, readers and the well stay public. Clicking **Vista** opens a contextual login dialog when needed, then saves the original judgment and keeps the research context. **Vistað** lists saved judgments; **Aðgangur** opens Clerk account management. No local credentials or user-profile database. Set up the provider and environment using [Authentication](docs/authentication.md) before activating accounts.
- **Search UI** — one band across the top of the page: the sources on the left, the search box level with them, and the date, year and sort controls on the right. The three are one act of asking, so they sit together; the date and sort controls are always on screen rather than behind a disclosure, where a date range nobody could see was silently narrowing people's results. Results are rows in one panel, 15 to a page, each row a link to the judgment. The query is kept in the URL (`/?q=…`) so a result page can be linked, bookmarked and reloaded.
- **The search is live, and always running** — there is no "nothing searched yet" state. The page opens on the Icelandic courts and shows them; changing anything — a source, a date, the sort, a subject tag — re-runs the search a moment later rather than waiting for a button. The search box still submits, because typing and pressing Enter is what a search box is for, but it is the one control that has to.
- **Sources as a hierarchy** — the panel is five groups (`src/lib/source-tree.ts`) over the same 57 source keys the API takes, tickable whole, with the 44 administrative bodies splitting further into appeal boards, ministries and valuation committees. A flat list of 57 checkboxes buried the courts under forty appeal boards and made "everything administrative" a forty-click operation. Searching inside the panel looks *into* collapsed groups and opens them, because a source whose group you cannot guess is otherwise unreachable; a `source-tree.test.ts` invariant keeps the tree covering every live source exactly once.
- **What is ticked is what is searched** — the selection is concrete, and it starts as the courts (`defaultSourceKeys`). It used to start empty and be *treated* as everything, which made the first click on a source read as removing it from a selection of 57 nobody had made. Each row also offers **aðeins**, which replaces the selection outright: "just the Supreme Court" is one click rather than four unticks. Unticking everything is an empty selection, not a wide one — the page says so and searches nothing, as the API has always insisted (`400 Select one or more sources to search.`).
- **Filters you can see** — a category ticked whole collapses to one navy chip; a category ticked in part names its sources, four of them before the rest fold behind `+ N fleiri`; every source ticked is one line of words (`Leitað í öllum heimildum · 57`) rather than 57 chips. The UI says *sources*, not *courts*: the panel is six courts, the Ombudsman, forty-four appeal boards and two journals, and calling all of that "courts" was wrong on three counts out of four.
...
src/
  ...
  lib/
    ...
    reglugerd.ts                 reglugerd.is HTML → the same, from either of the
                                 two markup generations the register is published in
  ...
  ingestion/
    ...
    adapters/
      ...
      reglugerd.ts               Icelandic regulations, from reglugerd.is; base
                                 regulations only, amendments read for their effects
      lagastod.ts                regulations → the acts they say they are made
                                 under; no network, reads what is already stored
...
| `LAGASAFN_FORCE=1` | lagasafn | Re-parse and rewrite even when nothing has changed. Rarely needed now: a parser that starts *storing* something new says so by bumping `PARSE_VERSION`, and the next scheduled run re-fetches everything behind it by itself. This is still the way to force a rewrite when the stored shape has not changed — checking a parser fix against the live corpus, say |
| `REGLUGERD_MAX_PAGES` | reglugerd | Register pages read per run (default 8; 30 entries a page) |
| `REGLUGERD_MAX_ITEMS` | reglugerd | Regulations fetched per run (default 400) |
| `REGLUGERD_ONLY` | reglugerd | Ingest a single regulation, e.g. `300/2020` — bypasses the register walk |
| `REGLUGERD_DELAY_MS` | reglugerd | Minimum gap between requests (default 5000, from reglugerd.is's own `Crawl-delay: 5`) |
| `REGLUGERD_FORCE=1` | reglugerd | Re-parse and rewrite even when nothing has changed |
| `REGLUGERD_API` | reglugerd | Override the API base URL |
| `LAGASTOD_MAX_REGULATIONS` | lagastod | Regulations scanned per run (default: all outstanding) |
| `LAGASTOD_BATCH_SIZE` | lagastod | Regulations held in memory at once (default 100) |
| `LAGASTOD_FORCE=1` | lagastod | Re-scan every regulation, ignoring the watermark |
...
That cheapness is also the reason the adapter carries a `PARSE_VERSION`. The index pins 903 of ~905 acts to one codex version, so the skip above fires before the parser runs — and widening the parse would otherwise reach only the handful of acts amended since, leaving the rest blank indefinitely. An act stored by an older parse version is re-fetched and re-saved regardless of its codex version, and the run says how many it is about to do, so bumping the constant *is* the backfill. The cost is one expensive run: ~900 fetches where the steady state is one.

**What the act page gives us besides the law.** Lagasafn prints two links between an act's number line and its first chapter — "Ferill málsins á Alþingi" and "Frumvarp til laga" — and footnotes each article with the act that amended it and the article of that act which did so (`L. 49/2016, 5. gr.`; lög nr. 91/1991 alone carries 236). Both are stored: `Act.ferillUrl` / `Act.billUrl`, and `Provision.footnotes`, one entry per numbered note. They are the chain from a provision to the bill commentary that explains it, and they are anchors Alþingi publishes rather than a match on number and title. The footnotes are not exclusively amendments — the regulations set under an article appear there too (`Rgl. 492/2001`) — so they are shown and stored as printed. Alþingi's own `/altext/` tree is not ingested; see *docs/regulations-and-travaux.md* §2.

### Reglugerðir

`reglugerd` ingests Icelandic regulations into the same table as lög, as
`jurisdiction: "is", docType: "regulation"`. They are the same shape — a titled
instrument with numbered articles, cited by number and year — so the act
reader, provision search and the act catalogue work on them unchanged.

**They are not lög, and `jurisdiction` can no longer say so on its own.**
Reglugerð nr. 300/2020 and lög nr. 300/2020 can both exist; regulation numbers
run past 1000 in a busy year and overlap act numbers completely. So every act
query that means lög must say `doc_type = 'act'`, and `corpusFilter()` in
`src/lib/acts.ts` is the one place that writes it. The same collision is why
the citation job now loads its act index with that filter: it is keyed by
number and year alone, because that is all `laga nr. 91/1991` gives, and a
...
does, and older ones are Word conversions where an article is a centred
paragraph reading "4. gr." and nothing says so. Both are parsed, and
`Act.structureSource` records which — `"structured"` or `"heuristic"` — because
a division inferred from centring is not a division the publisher asserted. The
act reader says so on the page, and nothing should hang a provision-level
citation link off a heuristic boundary.

Base regulations only are stored. `/current` returns a base regulation with its
amendments already folded in, so an amending regulation is read for its
`effects[]` — which names the base regulation it changes, and is the whole of
incremental change detection here — and then discarded. That keeps ~8,000
titles reading "breytingu á" out of search.

A cold start walks the register newest-first from a cursor, `REGLUGERD_MAX_PAGES`
pages at a time, wrapping at the end; at the default eight pages a run that is
about 60 runs to first full coverage. Like the Lagasafn adapter it carries a
`PARSE_VERSION`, so widening the parse backfills itself.

**Not yet built, and worth knowing about:** the *lagastoð* link. Every
regulation closes by naming the act it is made under — "Reglugerð þessi, sem
sett er samkvæmt 7. … gr. laga nr. 60/2007 …" — and `src/lib/legal-citations.ts`
already parses that form. Extracting it would give the act reader a list of the
regulations made under each act, which is the thing that makes this corpus
useful rather than a second silo. Judgment → regulation citation links are the
other half; `LEGISLATION_CITATION_RE` already matches `reglugerð nr. 1165/2016`
and has had nothing to resolve it to. Both are described in
*docs/regulations-and-travaux.md* §1.7.

### Frumvörp

`frumvorp` ingests the bill behind each act. It needs no listing: `Act.billUrl`
is already stored for every act whose Lagasafn page links one, so the adapter
selects acts that have a bill link and no bill stored, fetches it, and stores
it. `ctx.isKnown(source, officialUrl)` is the whole of the watermark — a bill
already held is skipped before it is fetched — so it needed no new column.

**What is in a þingskjal.** The bill's own articles, then "Athugasemdir við
lagafrumvarp þetta" (the general memorandum), then "Athugasemdir við einstakar
greinar frumvarpsins" — the commentary, article by article. That last section
is the reason to hold this corpus: frumvarp 1194 of the 130th þing, which
became jarðalög nr. 81/2004, comments on seventy-five articles individually.
`src/lib/thingskjal.ts` splits them, and refuses to attach a number to a
heading that names no single article ("Um 5. og 6. gr.", "Um ákvæði til
bráðabirgða III").

**Not every þingskjal has text.** Some are published only as a PDF, leaving a
page with a header, a document list and several thousand characters of site
navigation — the fjárlagafrumvarp is one. Those are recorded as a gap rather
...
`lagastod` reads the sentence every regulation writes about itself — "Reglugerð
þessi, sem sett er samkvæmt 7., 15. gr. a, 15. gr. b og 20. gr. laga nr.
60/2007 um Vatnajökulsþjóðgarð, öðlast þegar gildi" — and resolves it against
the acts this database holds, writing one `RegulationBasis` row per article
named. It touches no network: the text is already stored, so this is a query
and a regex over it, scheduled after `lagasafn` because it resolves against the
act index.

**Why it is not the general citation extractor.** `extractProvisionCitations`
in `src/lib/legal-citations.ts` requires an article and its act to be adjacent,
which is right for judgments and wrong here: a lagastoð clause routinely
enumerates several articles that share one trailing "gr." and one act
reference. On the sentence above it finds article 20 and misses 7, 15 a and
15 b — three quarters of the answer. `src/lib/lagastod.ts` reads the articles
as a run leading up to each act reference, which also keeps them with the right
act when a clause names two ("4. gr. hafnalaga nr. 61/2003 og 17. gr. laga nr.
41/2003 um siglingavernd").

**Why the anchor is the subject.** Measured over 84 regulations, a window
around any act citation near "sett", "stoð" or "heimild" matched plenty of
things that are not a basis: a cross-reference to a requirement "sem settar eru
fram í … 82. gr. laga um fjármálafyrirtæki", a clause saying a *tariff* is set
under an act, an ordinary "sbr. 47. gr. laga nr. 90/2003". What separates the
real clause is what it is about, so the extractor anchors on the subject —
"Reglugerð þessi", "Reglugerðin", "Reglur þessar" — followed by the verb. A
missed lagastoð is a regulation showing no enabling act; a false one asserts
that an act authorises something it does not, which is the worse failure and
the one the design is biased against.

On the same 84 regulations the extractor finds a stated basis in 83. Two forms
account for most of what the first attempt missed: the pre-1990 citation style,
where an act is named by number and date of assent ("laga nr. 49 17. maí 2005"
is lög nr. 49/2005), and the municipal byelaw, whose verb is "staðfestist"
rather than "sett". Both are handled, and both are in the tests.

An article that names no provision we hold does **not** drop the link: the row
is written against the act with a null provision, because the regulation does
rest on that act and saying so with less precision beats saying nothing.
Lagasafn publishes no provisions at all for about a tenth of acts in force, and
an article repealed since the regulation was made is gone from the consolidated
text by construction.

Incremental on `Act.lagastodScanHash`, the same one-column watermark
`Document.citationScanHash` gives the citation job: a regulation whose
`sourceHash` still matches is skipped, and each regulation's links and its
