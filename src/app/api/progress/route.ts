...
export async function GET() {
  ...
  // Pilot sources have nothing ingested yet; a permanently empty progress bar
  // would read as a stalled ingestion rather than an unbuilt adapter.
  const courts = sources
    .filter((s) => sourceByKey(s.key)?.status !== "pilot")
    .map((s) => ({
      key: s.key,
      name: s.name,
